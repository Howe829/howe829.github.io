import { requestFor, readMove } from './game.mjs';
...
export async function handle(request, env, upstreamFetch = fetch) {
  ...
  if(new URL(request.url).pathname !== '/api/move')return respond(404,'Not found');
  ...
  try {
    ...
    const result=readMove(await response.json(),input);
